// ===== Face Garden (p5.js + MediaPipe Tasks) — Angry via browDown + mouthPress =====
...
async function initFaceLandmarker() {
  ...
  faceLandmarker = await FaceLandmarker.createFromOptions(fileset, {
    ...
    runningMode: "VIDEO",
    numFaces: 1,
    outputFaceBlendshapes: true, // we use these for angry/happy, etc.
  });
}
...
function detectExpression(result) {
  ...
  // 2) Angry via browDown + mouthPress (averaged left/right)
  const browDownAvg = ema(
    "browDown",
    (getBlend(result, "browDownLeft") + getBlend(result, "browDownRight")) * 0.5,
    0.55
  );
  ...
  const ANGRY_T = 0.32; // try 0.28–0.40 depending on your camera/lighting

  // 3) Geometry fallback for happy/sad (scale-invariant via eye width)
  ...
  // 4) Decide expression (priority: happy > angry > sad > neutral)
  let expr = "neutral";
  ...
  // Angry if browDown + mouthPress strong (and not obviously smiling/jaw-open)
