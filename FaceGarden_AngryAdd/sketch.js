...
function draw() {
  background('lavender');
  // landmarker, createFaceLandMarker, Demo2
  updateAndDrawFaces();
}
...
class EyebrowFace extends Face {
  display() {
    ...
    let eyeOffsetX = this.size / 4;
    let eyeOffsetY = this.size / 6;
    let browLength = this.size / 5;

    // Angry left eyebrow (slants downward toward the center)
    line(this.x - eyeOffsetX - browLength/2, this.y - eyeOffsetY - 15, 
         this.x - eyeOffsetX + browLength/2, this.y - eyeOffsetY - 10);

    // Angry right eyebrow (slants downward toward the center)
    line(this.x + eyeOffsetX - browLength/2, this.y - eyeOffsetY - 10, 
         this.x + eyeOffsetX + browLength/2, this.y - eyeOffsetY - 15);

    // Straight Mouth
    stroke(0);
    strokeWeight(4);
    line(this.x - this.size / 4, this.y + this.size / 4, 
