...
function setup() {
  ...
  userStartAudio(); // new add for audio
}
...
function draw() {
  ...
  if (currentExpression === "happy") {
    ...
    new SadFace(width / 2, height / 2, 110).display();
     if (!sadSound.isPlaying()) sadSound.play();
    happySound.stop();
    angrySound.stop();
  } else if (currentExpression === "angry") {
    background(angryImage);
    new AngryFace(width / 2, height / 2, 110).display();
     if (!angrySound.isPlaying()) angrySound.play();
    happySound.stop();
    sadSound.stop();
  } else {
    fill(255);
...
let happySound, sadSound, angrySound, happyImage, sadImage, angryImage;

function preload() {
  soundFormats('mp3', 'wav'); // optional but helps
  happySound = loadSound('happyCut.mp3');
  sadSound = loadSound('sadCut.mp3');
  angrySound = loadSound('angryCut.mp3');
  happyImage = loadImage('3.png');
sadImage = loadImage('4.png');
angryImage = loadImage('5.png');
}

// make background and sound a part of the face class, cleanier code
