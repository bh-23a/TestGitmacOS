...
function drawSmiley(face) {
  ...
  // Face circle
  square(face.x, face.y, face.size);
  ...
  // Mouth, need mouth
  let mouthOffsetX = face.size / 48; //32
  let mouthOffsetY = face.size / 64; //48
  ...
  //ellipse(face.x - mouthOffsetX, face.y - mouthOffsetY, face.size / 5);
  //ellipse(face.x + mouthOffsetX, face.y - mouthOffsetY, face.size / 5);
  ...
  // create different classes to minimize code
  
  
  
  // Think about adding a for-loop
  // Motion capture - emotion vertices and if-else
